...
public sealed class SimulationEngine : IDisposable
{
    ...
    private int[] _sourceData = Array.Empty<int>();
    private int[] _visualData = Array.Empty<int>();
    ...
    private readonly List<SortEvent> _recordedEvents = new();
    private int[] _recordedInitialData = Array.Empty<int>();
    private string _recordedAlgorithmId = string.Empty;
    ...
    private int _parallelQueueDepth;
    private int _activeParallelTasks;
    private long _badPartitions;
    private double _pivotQualityAccum;
    private long _pivotQualitySamples;
    ...
    private int _parallelism = Math.Clamp(Environment.ProcessorCount, 1, 32);
    ...
    public void LoadData(int[] data)
    {
        ...
        lock (_stateLock)
        {
            ...
            _maxValue = Math.Max(1, _visualData.Length == 0 ? 1 : _visualData.Max());
    ...
    public void Start(ISortAlgorithm algorithm, string algorithmId = "", bool record = false, int parallelism = 1)
    {
        Stop();
        ...
        lock (_stateLock)
        {
            ...
            _maxValue = Math.Max(1, _visualData.Max());
            _memoryAccess.Resize(_visualData.Length);
            var configuredParallelism = Volatile.Read(ref _parallelism);
            var resolvedParallelism = Math.Max(1, parallelism <= 0 ? configuredParallelism : parallelism);
            var options = new SortOptions(MaxValue: _maxValue, EmitExtendedEvents: true, Parallelism: resolvedParallelism);
            _eventEnumerator = algorithm.Execute(_sourceData.AsSpan(), options).GetEnumerator();
        }

        lock (_recordLock)
        {
            _recordThisRun = record;
            _recordedEvents.Clear();
            _recordedInitialData = record ? _sourceData.ToArray() : Array.Empty<int>();
            _recordedAlgorithmId = record ? algorithmId : string.Empty;
    ...
    public void UpdateRuntimeControls(RuntimeControls controls)
    {
        ...
        Volatile.Write(ref _parallelism, Math.Max(1, controls.Parallelism));
        lock (_stateLock)
        {
    ...
    public int CopyDataTo(ref int[] destination, out int maxValue)
    {
        lock (_stateLock)
        {
            if (destination.Length != _visualData.Length)
            {
                destination = new int[_visualData.Length];
            }
    ...
    public int DrainAudioEvents(Span<AudioTrigger> target)
    {
        ...
        while (_audioQueue.TryDequeue(out var trigger))
        {
            ...
            // Drop oldest drained audio events to keep output aligned with current frame.
    ...
    {
        var pivotScore = 0.0;
        var sampleCount = Interlocked.Read(ref _pivotQualitySamples);
        if (sampleCount > 0)
        {
            pivotScore = Volatile.Read(ref _pivotQualityAccum) / sampleCount;
        }
        ...
        return new SortStatisticsSnapshot(
            ...
            ParallelQueueDepth: Interlocked.CompareExchange(ref _parallelQueueDepth, 0, 0),
            ActiveParallelTasks: Interlocked.CompareExchange(ref _activeParallelTasks, 0, 0),
            BadPartitions: Interlocked.Read(ref _badPartitions),
            PivotQualityScore: pivotScore);
    }
    ...
    private void TrackMemoryUnsafe(SortEvent ev)
    {
        switch (ev.Type)
        {
            ...
            case SortEventType.Swap:
                ...
                break;
            case SortEventType.Write:
            case SortEventType.MarkPivot:
            case SortEventType.MarkBucket:
            case SortEventType.BadPartition:
                _memoryAccess.Access(ev.I);
                break;
            case SortEventType.MarkRange:
            case SortEventType.PartitionInfo:
            case SortEventType.ParallelTaskStart:
            case SortEventType.ParallelTaskEnd:
                _memoryAccess.AccessPair(ev.I, ev.J);
                break;
            case SortEventType.ParallelQueueDepth:
                _memoryAccess.Access(ev.I);
                break;
        }
    }
    ...
    private void UpdateCounters(SortEvent ev)
    {
        ...
        switch (ev.Type)
        {
            ...
            case SortEventType.Write:
                ...
                break;
            case SortEventType.ParallelTaskStart:
                Interlocked.Increment(ref _activeParallelTasks);
                break;
            case SortEventType.ParallelTaskEnd:
            {
                var active = Interlocked.Decrement(ref _activeParallelTasks);
                if (active < 0)
                {
                    Interlocked.Exchange(ref _activeParallelTasks, 0);
                }
                break;
            }
            case SortEventType.ParallelQueueDepth:
                Interlocked.Exchange(ref _parallelQueueDepth, Math.Max(0, ev.Value));
                break;
            case SortEventType.BadPartition:
                Interlocked.Increment(ref _badPartitions);
                break;
            case SortEventType.PartitionInfo:
            {
                var q = Math.Clamp(ev.Value / 1000.0, 0.0, 1.0);
                _pivotQualityAccum += q;
                Interlocked.Increment(ref _pivotQualitySamples);
                break;
            }
        }
    }
    ...
    private void MaybeEmitAudioEvent(SortEvent ev)
    {
        ...
        if (ev.Type is not (SortEventType.Compare or SortEventType.Swap or SortEventType.Write or SortEventType.MarkPivot))
        {
            return;
        }
        ...
        _audioQueue.Enqueue(new AudioTrigger(ev.Type, value, _maxValue, pan));
    ...
    private void MaybeRecordEvent(SortEvent ev)
    {
        if (!_recordThisRun)
        {
            return;
    ...
    private int ResolveEventValue(SortEvent ev)
    {
        lock (_stateLock)
        {
            ...
            {
                SortEventType.Write => Math.Clamp(ev.Value, 0, _maxValue),
                SortEventType.Swap => Math.Clamp(MaxAtIndices(ev.I, ev.J), 0, _maxValue),
                SortEventType.Compare => Math.Clamp(AtIndex(ev.I), 0, _maxValue),
                SortEventType.MarkPivot => Math.Clamp(ev.Value == 0 ? AtIndex(ev.I) : ev.Value, 0, _maxValue),
                _ => Math.Clamp(ev.Value, 0, _maxValue)
            };
        }
    }
    ...
    private float ResolveEventPan(SortEvent ev)
    {
        lock (_stateLock)
        {
            ...
            var idx = ev.Type switch
            {
                SortEventType.Write => ev.I,
                SortEventType.Swap => ev.I >= 0 && ev.J >= 0 ? (ev.I + ev.J) / 2 : Math.Max(ev.I, ev.J),
                SortEventType.Compare => ev.I >= 0 && ev.J >= 0 ? (ev.I + ev.J) / 2 : Math.Max(ev.I, ev.J),
                SortEventType.MarkPivot => ev.I,
                _ => ev.I >= 0 ? ev.I : ev.J
            };

            idx = Math.Clamp(idx, 0, _visualData.Length - 1);
            var t = idx / (float)Math.Max(1, _visualData.Length - 1);
            return t * 2.0f - 1.0f;
        }
    }
    ...
    {
        return IsValidIndex(index) ? _visualData[index] : 0;
    }

    private int MaxAtIndices(int i, int j)
    {
        var a = IsValidIndex(i) ? _visualData[i] : 0;
        var b = IsValidIndex(j) ? _visualData[j] : 0;
        return Math.Max(a, b);
    }
    ...
    private static bool PassesDetail(SortEventType type, DetailLevel level)
    {
        return level switch
        {
            DetailLevel.L1 => type is SortEventType.Swap or SortEventType.Write,
            DetailLevel.L2 => type is SortEventType.Compare or SortEventType.Swap or SortEventType.Write,
    ...
    private void ResetStats()
    {
        ...
        Interlocked.Exchange(ref _cacheHits, 0);
        Interlocked.Exchange(ref _cacheMisses, 0);
        Interlocked.Exchange(ref _parallelQueueDepth, 0);
        Interlocked.Exchange(ref _activeParallelTasks, 0);
        Interlocked.Exchange(ref _badPartitions, 0);
        Volatile.Write(ref _pivotQualityAccum, 0.0);
        Interlocked.Exchange(ref _pivotQualitySamples, 0);
    ...
    public bool TryGetLastReplayLog(out SortReplayLog? replayLog)
    {
        lock (_recordLock)
        {
            if (_recordedInitialData.Length == 0 || _recordedEvents.Count == 0)
            {
                replayLog = null;
                return false;
            }

            replayLog = new SortReplayLog(
                AlgorithmId: _recordedAlgorithmId,
                CreatedUtc: _recordedAtUtc,
                InitialData: _recordedInitialData.ToArray(),
                Events: _recordedEvents.ToArray());
            return true;
        }
    }

    public bool TryGetRecordedRun(out string algorithmId, out int[] initialData, out SortEvent[] events, out DateTime createdUtc)
    {
        lock (_recordLock)
        {
            if (_recordedInitialData.Length == 0 || _recordedEvents.Count == 0)
            {
                ...
                initialData = Array.Empty<int>();
                events = Array.Empty<SortEvent>();
